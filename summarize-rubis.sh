...
do
	...
	thr=`cat ../logs/log-rubis-$vm1r-$vm2r-$vm1c-$vm2c-$size-$i.html | sed -n 62'p' | awk {'print $5'} | tr '<B>' ' '| awk {'print $2'}`
	rt=`cat ../logs/log-rubis-$vm1r-$vm2r-$vm1c-$vm2c-$size-$i.html | sed -n 61'p' | awk {'print $9'} | tr '<B>' ' '| awk {'print $2'}`
	echo $vm1r,$vm2r,$vm1c,$vm2c,$size,$gvm1,$gvm2,$uvm1,$uvm2,$avm1,$avm2,$qvm1,$qvm2,$quvm1,$quvm2,$rt,$thr >> summary.csv
done
done
done
