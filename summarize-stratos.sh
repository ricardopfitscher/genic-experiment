...
do
	...
	thr=`cut -f 1 -d , $path/log-stratos-$vm1r-$vm2r-$vm1c-$vm2c-$size-$i | cut -f 2 -d : | ./est-media.awk `
	rt=`cut -f 2 -d , $path/log-stratos-$vm1r-$vm2r-$vm1c-$vm2c-$size-$i | cut -f 2 -d : | ./est-media.awk `
	echo $vm1r,$vm2r,$vm1c,$vm2c,$size,$gvm1,$gvm2,$uvm1,$uvm2,$avm1,$avm2,$qvm1,$qvm2,$quvm1,$quvm2,$rt,$thr >> summary.csv
done
done
done
